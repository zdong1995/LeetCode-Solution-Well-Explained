class Solution {
    ...
    private TreeNode construct(int[] pre, int preLeft, int preRight, int[] post, int postLeft, int postRight,
        ...
        root.left = construct(pre, preLeft + 1, preLeft + leftSize, post, postLeft, leftRootIdx, nodeIndex);
        root.right = construct(pre, preLeft + leftSize + 1, preRight, post, leftRootIdx + 1, postRight, nodeIndex);
        return root;
    }
}